#!/usr/bin/env python3
"""
Deploy 3 local LLM endpoints with llama.cpp + tmux — GPU-enabled (v5).

Changes vs v4:
  * Builds llama.cpp with CUDA (-DGGML_CUDA=ON) when an NVIDIA GPU + nvcc
    (CUDA toolkit) are present.
  * Launches llama-server with -ngl (GPU layer offload) so the models run on
    the GPU instead of CPU. Default offloads all layers (99).
  * New flags: --rebuild (force recompile, needed to switch CUDA on) and
    --gpu-layers N (set -ngl; use 0 to force CPU).
...
Usage:
  # First time on GPU: recompile with CUDA, then start on GPU
  python3 deploy_local_models_v5.py --restart --rebuild --wait-minutes 30
  python3 deploy_local_models_v5.py --status
  python3 deploy_local_models_v5.py --stop
...
GPU_LAYERS = 99  # -ngl: layers to offload to GPU. 99 = all (fits easily for 2-4B).
...
def ensure_llama_cpp(rebuild: bool = False) -> Path:
    ...
    if have_gpu and not exists("nvcc"):
        print("WARNING: GPU detected but 'nvcc' (CUDA toolkit) not found.")
        print("         Building CPU-only. Install the CUDA toolkit, e.g.:")
        print("           sudo apt-get install -y nvidia-cuda-toolkit")
        print("         then re-run with --rebuild to enable the GPU.")
...
def free_port(port: int, timeout: int = 15) -> bool:
    """Ensure nothing is listening on `port` before we start a new server.

    Kills any llama-server still bound to it (avoids the restart bind race)
    and waits for the OS to release the port.
    """
...
def start_model(model: ModelSpec, llama_server: Path, restart: bool, redownload: bool, gpu_layers: int) -> None:
    ...
    if not free_port(model.port):
        print(f"WARNING: port {model.port} is still in use; {model.key} may fail to bind.")
    ...
    command = [
        ...
        "-ngl", str(gpu_layers),   # GPU offload: 99 = all layers, 0 = CPU only
        "--jinja",                 # use the model's chat template; lets clients
                                   # pass chat_template_kwargs (e.g. to disable
                                   # a model's "thinking"/reasoning output)
    ]
    ...
    print(f"Started: {model.session} (-ngl {gpu_layers})")
...
def deploy(models: list[ModelSpec], restart: bool, wait_minutes: int, redownload: bool,
           rebuild: bool, gpu_layers: int) -> None:
    ensure_dirs()
    ensure_tmux()
    llama_server = ensure_llama_cpp(rebuild)
    for model in models:
        start_model(model, llama_server, restart, redownload, gpu_layers)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Deploy local GGUF model servers with llama.cpp + tmux (GPU-enabled).")
    ...
    parser.add_argument("--status", action="store_true", help="Show GPU, tmux sessions, ports, and endpoints")
    ...
    parser.add_argument("--gpu-layers", type=int, default=GPU_LAYERS, help="Layers to offload to GPU (-ngl). 0 = CPU only")
    ...
    try:
        if args.status:
            ...
            deploy(args.models, args.restart, args.wait_minutes, args.redownload, args.rebuild, args.gpu_layers)
        return 0
